%define module fcdslslusb
%define version 3.11.05
%define card "AVM GmbH|FtitzCard USB DSL SL USB Analog"

Summary: dkms package for %{module} driver
Name: dkms-%{module}
Version: %{version}
Release: %mkrel 7
Source0: ftp://ftp.avm.de/cardware/fritzcrd.dsl_sl_usb/linux/suse.93/fcdslslusb-suse93-3.11-05.tar.bz2
Source1: dkms-fcdslslusb-use-autoconf-header.patch
Source2: dkms-fcdslslusb-dont-redefine-uintptr_t.patch
Source3: dkms-fcdslslusb-remove-usb_driver_owner.patch
Source4: dkms-fcdslslusb-remove-DECLARE_MUTEX_LOCKED.patch
Source5: dkms-fcdslslusb-update-INIT_WORK-changes.patch
Source6: dkms-fcdslslusb-update-usb_complete_t-functions.patch
...
CLEAN="make clean"
AUTOINSTALL="yes"
PATCH[0]="dkms-fcdslslusb-use-autoconf-header.patch"
PATCH_MATCH[0]="^2\.6\.(19)|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[1]="dkms-fcdslslusb-dont-redefine-uintptr_t.patch"
PATCH_MATCH[1]="^2\.6\.(2[4-9])|([3-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[2]="dkms-fcdslslusb-remove-usb_driver_owner.patch"
PATCH_MATCH[2]="^2\.6\.(1[6-9])|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[3]="dkms-fcdslslusb-remove-DECLARE_MUTEX_LOCKED.patch"
PATCH_MATCH[3]="^2\.6\.(2[4-9])|([3-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[4]="dkms-fcdslslusb-update-INIT_WORK-changes.patch"
PATCH_MATCH[4]="^2\.6\.([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[5]="dkms-fcdslslusb-update-usb_complete_t-functions.patch"
PATCH_MATCH[5]="^2\.6\.(19)|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
EOF

tar c . | tar x -C $RPM_BUILD_ROOT/usr/src/%module-%version-%release/

for p in %{_sourcedir}/dkms-fcdslslusb-use-autoconf-header.patch \
         %{_sourcedir}/dkms-fcdslslusb-dont-redefine-uintptr_t.patch \
         %{_sourcedir}/dkms-fcdslslusb-remove-usb_driver_owner.patch \
         %{_sourcedir}/dkms-fcdslslusb-remove-DECLARE_MUTEX_LOCKED.patch \
         %{_sourcedir}/dkms-fcdslslusb-update-INIT_WORK-changes.patch \
         %{_sourcedir}/dkms-fcdslslusb-update-usb_complete_t-functions.patch;
do
